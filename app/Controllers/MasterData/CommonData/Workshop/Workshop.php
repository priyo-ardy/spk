...
class Workshop extends BaseController
{
    ...
    {
        $this->module = "Workshop Management";
    ...
    function loadTable()
    {
        log_action($this->module, "Table", "info", current_url(), "Generating workshop list");
    ...
    public function index()
    {
        ...
        return view('MasterData/CommonData/Workshop/index', $data);
    }
    ...
    function saveData()
    {
        ...
        try {
            $id = generate_uuid();
            $code = $this->masterModel->generateCode('m_workshop', 'code', 'WRS-', 4);
            ...
            $rules = [
                'data_name' => [
                    ...
                    'errors' => [
                        ...
                        'min_length' => "The minimum character of workshop name is {param}",
                        'max_length' => "The maximum character of workshop name is {param}",
                    ]
                ]
            ];
            ...
            if (!$insert) {
                log_action($this->module, $aksi, 'error', current_url(), "Save failed, there was an error during processing your request", '', json_encode([
                    'data' => $this->workshopModel->errors()
                ]));
    ...
    function getData() {
        ...
        try{
            ...
            $id_workshop = dekripsi($json_data['token']);

            $get_data = $this->workshopModel->getDataById($id_workshop);
            if(!$get_data){
                log_action($this->module, $aksi, "error", current_url(), "Workshop data not found");

                return pesan(ResponseInterface::HTTP_NOT_FOUND, "Workshop data not found");
            }
            ...
            return pesan(ResponseInterface::HTTP_OK, "Workshop data found", $data);
        } catch(\Exception $e){
        ...
        try{
            ...
            $rules = [
                'data_token' => [
                    'rules' => 'required',
                    'errors' => [
                        'required' => "Workshop token is required"
                    ]
                ],
                'data_code' => [
                    'rules' => 'required',
                    'errors' => [
                        'required' => "Workshop code is required"
                    ]
                ],
                'data_name' => [
                    'rules' => 'required|min_length[1]|max_length[150]',
            ...
            $update = $this->workshopModel->update($category_id, $data);
            if(!$update){
    ...
    function deleteData() {
        ...
        try{
            ...
            $check_data = $this->workshopModel->getDataById($id_workshop);
            if(!$check_data){
                log_action($this->module, $aksi, "error", current_url(), "Workshop data not found");
                return pesan(ResponseInterface::HTTP_NOT_FOUND, "Workshop data not found");
            }

            $delete = $this->workshopModel->delete($id_workshop);
            if(!$delete){
