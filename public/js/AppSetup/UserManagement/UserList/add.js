...
function checkUsername() {
  if (dataForm.user_name.value === "") {
    ...
    try {
      fetchData(
        baseurl + "/users/check_user",
        "POST",
        JSON.stringify({ username: dataForm.user_name.value, token: "" })
      )
        .then((result) => {
...
function checkUserPhone() {
  try {
    fetchData(
      baseurl + "/users/check_phone",
      "POST",
      JSON.stringify({ user_phone: dataForm.phone_number.value, token: "" })
    )
      .then((result) => {
...
function checkUserEmail() {
  try {
    if (isValidEmail(dataForm.email_address.value)) {
      fetchData(
        baseurl + "/users/check_email",
        "POST",
        JSON.stringify({ user_email: dataForm.email_address.value, token: "" })
      )
        .then((result) => {
