...
function loadTable() {
  $("#dataTable").DataTable({
    ...
    order: [],
    ajax: {
      url: baseurl + "/equipment_type/table",
      type: "POST",
      data: "raw",
      action: "calls",
    },
...
function deleteData(token) {
  try {
    hapusData("/equipment_type/delete", token);
  } catch (e) {
    pesanError(e.message);
  }
}
