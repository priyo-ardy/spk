...
buttons.save.addEventListener("click", (e) => {
  if (dataForm.name.value === "") {
    ...
    try {
      loading();
      fetchData(baseurl + "/lokasi/save", "POST", new FormData(formData))
        .then((result) => {
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/lokasi/get", "POST", JSON.stringify({ token: token }))
      .then((result) => {
...
function validasiUpdate() {
  ...
  if (dataForm.token.value.trim() === "") {
    pesanWarning("Location token is required");
    isValid = false;
  }
...
buttons.update.addEventListener("click", (e) => {
  if (validasiUpdate()) {
    try {
      loading();
      fetchData(baseurl + "/lokasi/update", "POST", new FormData(formData))
        .then((result) => {
          pesanSukses(result.message);
          resetForm();
          refreshTable();
