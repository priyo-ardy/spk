...
const buttons = {
  ...
  donwload: document.getElementById("btnDownload"),
...
buttons.save.addEventListener("click", (e) => {
  if (dataForm.name.value === "") {
    ...
    try {
      loading();
      fetchData(baseurl + "/departemen/save", "POST", new FormData(formData))
        .then((result) => {
...
function validasiUpdate() {
  ...
  if (dataForm.token.value.trim() === "") {
    pesanWarning("Department token is required");
    isValid = false;
  }
...
buttons.update.addEventListener("click", (e) => {
  if (validasiUpdate()) {
    try {
      loading();
      fetchData(baseurl + "/departemen/update", "POST", new FormData(formData))
        .then((result) => {
          pesanSukses(result.message);
          resetForm();
          refreshTable();
...
function deleteData(token) {
  try {
    hapusData("/departemen/delete", token);
  } catch (e) {
    pesanError(e.message);
  }
}
