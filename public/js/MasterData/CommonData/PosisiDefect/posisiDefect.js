...
const dataForm = {
  ...
  name: document.getElementById("data_posisi"),
...
function loadTable() {
  $("#dataTable").DataTable({
    ...
    order: [],
    ajax: {
      url: baseurl + "/posisi_defect/table",
      type: "POST",
      data: "raw",
      action: "calls",
    },
...
buttons.save.addEventListener("click", (e) => {
  if (dataForm.name.value === "") {
    ...
    try {
      loading();
      fetchData(baseurl + "/posisi_defect/save", "POST", new FormData(formData))
        .then((result) => {
...
function deleteData(token) {
  try {
    hapusData("/posisi_defect/delete", token);
  } catch (e) {
    pesanError(e.message);
  }
}
