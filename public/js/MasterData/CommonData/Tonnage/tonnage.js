...
buttons.save.addEventListener("click", (e) => {
  if (dataForm.name.value === "") {
    ...
    try {
      loading();
      fetchData(baseurl + "/tonnage/save", "POST", new FormData(formData))
        .then((result) => {
...
function validasiUpdate() {
  ...
  if (dataForm.token.value.trim() === "") {
    pesanWarning("Tonnage token is required");
    isValid = false;
  }
...
buttons.update.addEventListener("click", (e) => {
  if (validasiUpdate()) {
    try {
      loading();
      fetchData(baseurl + "/tonnage/update", "POST", new FormData(formData))
        .then((result) => {
          pesanSukses(result.message);
          resetForm();
          refreshTable();
...
function deleteData(token) {
  try {
    hapusData("/tonnage/delete", token);
  } catch (e) {
    pesanError(e.message);
  }
}
